...
class CutMixTransform:
    ...
    def __call__(self, batch):
        ...
        images_copy[:, :, bbx1:bbx2,
                    bby1:bby2] = images[indices, :, bbx1:bbx2, bby1:bby2]
...
def prepare_dataloaders(dataset_path, test_path):
    ...
    # Sanitize dataset paths - extract the actual path if it contains debug output
    if '\n' in dataset_path:
        import re
        match = re.search(r'TRAIN_PATH: (.*?)(?:\n|$)', dataset_path)
        if match:
            dataset_path = match.group(1).strip()
            print(f"⚠️ Sanitized training path: {dataset_path}")

    # Verify the paths actually exist
    if not os.path.exists(dataset_path):
        print(
            f"⚠️ ERROR: Training dataset path does not exist: {dataset_path}")
        # Try to construct a valid path
        alternative_path = os.path.join("./extracted/emotion", "train")
        if os.path.exists(alternative_path):
            print(f"⚠️ Using alternative training path: {alternative_path}")
            dataset_path = alternative_path
        else:
            raise FileNotFoundError(
                f"Could not find training dataset: {dataset_path}")

    if not os.path.exists(test_path):
        print(f"⚠️ ERROR: Testing dataset path does not exist: {test_path}")
        alternative_path = os.path.join("./extracted/emotion", "test")
        if os.path.exists(alternative_path):
            print(f"⚠️ Using alternative testing path: {alternative_path}")
            test_path = alternative_path
        else:
            raise FileNotFoundError(
                f"Could not find testing dataset: {test_path}")

    # Print the final path for clarity
    print(f"🔹 FINAL TRAINING PATH: {dataset_path}")
    print(f"🔹 Using original dataset (MODEL_BALANCE_DATASET=0)")
    os.environ['MODEL_BALANCE_DATASET'] = '0'

    # Remove validation files that don't contain 'PublicTest' in their names
    print("🔹 Checking validation dataset for files without 'PublicTest' in their names")
    files_to_delete = []
    deleted_count = 0

    # Walk through the validation dataset directory
    for root, dirs, files in os.walk(test_path):
        for file in files:
            if file.endswith(('.jpg', '.jpeg', '.png')):
                if 'PublicTest' not in file:
    ...
    if files_to_delete:
        print(
            f"🔹 Found {len(files_to_delete)} files without 'PublicTest' in their names to delete")
        ...
        print(
            f"✅ Successfully deleted {deleted_count} files from validation dataset")
    else:
        print("✅ All validation files already contain 'PublicTest' in their names")
    ...
    try:
        ...
        print(f"⚠️ Error creating training dataset: {str(e)}")
        raise
    ...
    try:
        ...
        print(f"⚠️ Error creating validation dataset: {str(e)}")
        raise
    ...
    try:
        ...
        print(f"⚠️ Error checking class indices: {str(e)}")
    ...
    print(f"Total samples in validation dataset: {len(val_dataset.samples)}")
...
def tta_evaluate(model, val_loader, criterion, device, num_augments=TTA_NUM_AUGMENTS):
    ...
    # Option to completely skip TTA and just do standard eval
    skip_tta = os.getenv("SKIP_TTA", "0") == "1"
    ...
    if skip_tta or ultra_fast:
        ...
        # Use at most 2 augmentations in fast mode
        actual_augments = min(2, num_augments)
            ...
            if actual_augments > 1:
                for i in range(actual_augments - 1):
                    ...
                    # Only add shifts in full evaluation mode
                        ...
                        shift_x, shift_y = random.randint(
                            -2, 2), random.randint(-2, 2)
                        if shift_x != 0 or shift_y != 0:
                            aug_images = torch.roll(aug_images, shifts=(
                                shift_x, shift_y), dims=(2, 3))
...
def ensemble_evaluate(device, val_loader, criterion):
    ...
    print(
        f"🔹 Using {len(top_checkpoints)} best model checkpoints for ensemble evaluation")
    models = []
    ...
    class_accuracies = [100.0 * correct_count / max(
        1, total_count) for correct_count, total_count in zip(class_correct, class_total)]
...
class SWA:
    ...
    def __init__(self, model, swa_start=10, swa_freq=5, device=None):
        ...
        self.device = device if device is not None else next(
    ...
    def update(self, epoch):
        ...
        if epoch >= self.swa_start and (epoch - self.swa_start) % self.swa_freq == 0:
            ...
            for param_q, param_k in zip(self.model.parameters(), self.swa_model.parameters()):
                param_k.data = (param_k.data * self.swa_n +
                                param_q.data) / (self.swa_n + 1)

            print(
                f"✅ SWA model updated at epoch {epoch} (model count: {self.swa_n})")
    ...
    def finalize(self):
        ...
        def update_bn(model, loader, device):
            ...
            print(
                f"  Using {'fast' if fast_finalize else 'standard'} SWA finalization with {max_batches} batches")
            ...
            with torch.no_grad():
                for batch_idx, (images, _) in enumerate(loader):
                    ...
                    if batch_idx % 5 == 0:
                        print(
                            f"  Batch {batch_idx+1}/{min(max_batches, len(loader))}", end="\r")
            ...
            print(
                f"  Used {min(batch_idx+1, max_batches)} batches for BN calibration    ")
