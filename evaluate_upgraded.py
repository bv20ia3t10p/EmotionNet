#!/usr/bin/env python3
"""
Evaluation script for UltraAdvancedFER model
Compatible with train_upgraded.py architecture
"""
...
# Import the UltraAdvancedFER model from train_upgraded.py
import sys
sys.path.append('.')

try:
    from train_upgraded import (UltraAdvancedFER, FERPlusDataset, 
                               AdvancedPatchEmbedding, SqueezeExciteAttention,
                               PyramidFeatureFusion, AdaptiveEnsembleHead)
    print("✅ Successfully imported UltraAdvancedFER model")
except ImportError as e:
    print(f"❌ Error importing model: {e}")
    print("Make sure train_upgraded.py is in the current directory")
    sys.exit(1)

def load_ultraadvanced_model(checkpoint_path, device):
    """Load UltraAdvancedFER model from checkpoint"""
    print(f"Loading UltraAdvanced model from {checkpoint_path}...")
...
def evaluate_model(model, dataloader, device, dataset_name="Test"):
    ...
    with torch.no_grad():
        for images, hard_labels, soft_labels in tqdm(dataloader, desc=f'{dataset_name} Evaluation'):
            ...
            # Handle potential tuple output from training mode
...
def plot_confusion_matrix(results, emotion_names, save_path="confusion_matrix_upgraded.png"):
    ...
    plt.title('Confusion Matrix - UltraAdvanced FER Model')
...
def plot_class_distribution(results, emotion_names, save_path="class_distribution_upgraded.png"):
    ...
    bars = plt.bar([emotion_names[i] for i in unique], counts, color='skyblue', alpha=0.7)
...
def analyze_model_architecture(model):
    ...
    print(f"Number of Heads:      {len(model.blocks[0]['attn'].qkv.weight) // model.embed_dim // 3}")
    print(f"Number of Classes:    {model.num_classes}")

def main():
    parser = argparse.ArgumentParser(description='Evaluate UltraAdvanced FER Model')
    ...
    # Load model
    model = load_ultraadvanced_model(args.model_path, device)
    ...
    print("="*60)
    print(f"Model: UltraAdvanced FER")
