#!/usr/bin/env python3
"""
Evaluation script for EffectiveFER model
Compatible with train_upgraded_fixed.py architecture
"""
...
# Import the EffectiveFER model from train_upgraded_fixed.py
import sys
sys.path.append('.')

try:
    from train_upgraded_fixed import (EffectiveFER, FERPlusDataset, 
                                     EfficientPatchEmbedding, FocalLoss)
    print("✅ Successfully imported EffectiveFER model")
except ImportError as e:
    print(f"❌ Error importing model: {e}")
    print("Make sure train_upgraded_fixed.py is in the current directory")
    sys.exit(1)

def load_effective_model(checkpoint_path, device):
    """Load EffectiveFER model from checkpoint"""
    print(f"Loading Effective model from {checkpoint_path}...")
    ...
    model = EffectiveFER(
        ...
        num_heads=8,    # Proper attention heads
        dropout=0.1
    ).to(device)
...
def plot_confusion_matrix(results, emotion_names, save_path="confusion_matrix_effective.png"):
    ...
    plt.title('Confusion Matrix - Effective FER Model')
...
def plot_class_distribution(results, emotion_names, save_path="class_distribution_effective.png"):
    ...
    bars = plt.bar([emotion_names[i] for i in unique], counts, color='lightgreen', alpha=0.7)
...
def analyze_model_architecture(model):
    ...
    print(f"\nArchitecture Details:")
    print(f"Image Size:           {model.patch_embed.img_size}x{model.patch_embed.img_size}")
    ...
    print(f"Number of Heads:      {model.blocks[0].self_attn.num_heads}")
...
def plot_performance_comparison(results, baseline_acc=83.84):
    ...
    categories = ['Baseline\n(Original)', 'Effective FER\n(This Model)']
    accuracies = [baseline_acc, accuracy]
    colors = ['skyblue', 'lightgreen' if accuracy > baseline_acc else 'lightcoral']
    ...
    plt.tight_layout()
    plt.savefig('performance_comparison_effective.png', dpi=300, bbox_inches='tight')
    plt.show()
    print("Performance comparison saved to performance_comparison_effective.png")

def main():
    parser = argparse.ArgumentParser(description='Evaluate Effective FER Model')
    parser.add_argument('--model_path', type=str, default='best_effective_fer.pth',
    ...
    # Load model
    model = load_effective_model(args.model_path, device)
    ...
    plot_class_distribution(results, emotion_names)
    ...
    print("="*60)
    print(f"Model: Effective FER (Simplified Architecture)")
