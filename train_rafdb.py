...
def parse_args():
    parser = argparse.ArgumentParser(description='Train GReFEL on RAF-DB dataset')
    # Dataset
    parser.add_argument('--data_dir', type=str, default='./rafdb/DATASET', help='Path to RAF-DB dataset directory')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size (reduced for RAF-DB)')
    ...
    parser.add_argument('--drop_rate', type=float, default=0.2, help='Dropout rate (increased for RAF-DB)')
    
    # Training hyperparameters
    parser.add_argument('--epochs', type=int, default=80, help='Number of training epochs')
    parser.add_argument('--lr', type=float, default=1e-4, help='Peak learning rate (increased for RAF-DB)')
    parser.add_argument('--min_lr', type=float, default=1e-6, help='Minimum learning rate')
    parser.add_argument('--weight_decay', type=float, default=0.01, help='Weight decay (increased for RAF-DB)')
    parser.add_argument('--label_smoothing', type=float, default=0.1, help='Label smoothing factor')
    parser.add_argument('--mixup_alpha', type=float, default=0.4, help='Mixup alpha parameter (increased for RAF-DB)')
    parser.add_argument('--device', type=str, default='cuda', help='Device to use')
    parser.add_argument('--output_dir', type=str, default='checkpoints_rafdb', help='Output directory')
...
def get_rafdb_loaders(args):
    """Create train and validation data loaders for RAF-DB."""
    ...
    val_dataset = RAFDBDataset(
        root_dir=args.data_dir,
        split='val',
        val_split=0.1  # Use same split ratio
    )
    ...
    train_labels = [int(train_dataset.data['label'][i])-1 for i in range(len(train_dataset))]
    val_labels = [int(val_dataset.data['label'][i])-1 for i in range(len(val_dataset))]
    ...
    for i in range(7):  # 7 emotion classes
        ...
        print(f"{emotion_names[i]}: {count} samples ({count/len(train_dataset)*100:.1f}%)")
...
def get_lr_scheduler(optimizer, args):
    ...
    def lr_lambda(epoch):
        if epoch < args.warmup_epochs:
            ...
            return args.min_lr / args.lr + (1 - args.min_lr / args.lr) * \
...
def main():
    ...
    # Create model - RAF-DB has 7 basic emotion classes
    model = GReFEL(
        num_classes=7,  # RAF-DB basic emotions
    ...
    # Get data loaders
    train_loader, val_loader = get_rafdb_loaders(args)
    
    # Loss function
    criterion = EnhancedGReFELLoss(
        num_classes=7,  # RAF-DB basic emotions
        label_smoothing=args.label_smoothing,
        geo_weight=0.02,  # Increased for RAF-DB
        reliability_weight=0.15  # Increased for RAF-DB
    )
    ...
    print(f"Training on RAF-DB with configuration:")
